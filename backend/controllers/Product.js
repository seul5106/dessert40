...
module.exports = (app) => {
  ...
  router.get("/product", async (req, res, next) => {
    ...
    if (cate === "all") {
      ...
      try {
        ...
        // 데이터 조회
        let sql2 =
          "SELECT product_code, product_name, product_price, product_stock, product_img, product_categorie, product_desc, product_nutri, product_allergy, product_date, product_state FROM products  ORDER BY product_code DESC";
  ...
  router.delete("/product/delete", async(req, res, next) => {
    ...
    try {
      ...
      //데이터 삭제하기
      let sql = "DELETE FROM products WHERE product_code = ?";
      const [result1] = await dbcon.query(sql, key);
      logger.debug([result1]);
      if (result1.affectedRows < 1) {
        throw new Error("비활성화된 데이터가 없습니다.");
