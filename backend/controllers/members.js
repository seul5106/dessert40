...
module.exports = (app) => {
    ...
    router.get("/admin/list", async (req, res, next) => {
        ...
        try {
            ...
            // 데이터 조회
            let sql2 = "SELECT member_code, member_name, member_id, member_email, member_phone, concat(member_postcode,' ',member_addr1,' ',member_addr2) as totalAdd, reg_date, is_out FROM members ORDER BY reg_date DESC";
    ...
    //로그아웃 기능
    router.get("/members/logout", async (req, res, next) => {
        req.session.destroy(() => {
    ...
    //멤버 탈퇴 버튼
    router.post("/members/is_out", async (req, res, next) => {
        const key = req.post("memberCode");
        logger.warn("멤버코드 = " + key)
        if (key === undefined) {
            //400 Bad Request -> 잘못된 요청
            return next(new Error(400));
        }
        try {
            ...
            //데이터 N에서 Y로 변경
            let sql = "UPDATE members SET is_out = 'Y'  WHERE member_code = ?";
