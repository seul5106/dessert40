...
module.exports = (app) => {
  ...
  router.post("/order", async (req, res, next) => {
    ...
    // 취소요청 여부
    const rqCancel = req.post("rq_cancel");
    // 아임포트 주문번호
    ...
    try {
      ...
      await dbcon.query(sql1, inputData1);
  ...
  router.post("/cart/save", async (req, res, next) => {
    ...
    try {
      ...
      await dbcon.query(sql, inputData);
    } catch (err) {
      return next(err);
    } finally {
      dbcon.end();
  ...
  router.get("/order_confirm", async (req, res, next) => {
    ...
    const merchantUid = req.get("merchant_uid");
    // get파라미터로 받은 아임포트 주문번호
    const impUid = req.get("imp_uid");
    // 세션에 저장되어있는 회원번호코드
    const memberCode = req.session.memberInfo.member_code;
    ...
    try {
      ...
      const sql1 =
        "SELECT merchant_uid, ordered_product_name, ordered_product_img, ordered_product_count, ordered_product_price from ordered_product WHERE merchant_uid = ?";
      const inputData1 = [merchantUid];
      const [result1] = await dbcon.query(sql1, inputData1);

      //ordered_product JSON저장
      orderedProduct = result1;

      //orders테이블의 imp_uid컬럼을 업데이트하며 동시에 order_state값을 Y로 바꿔준다
      const sql2 =
        "UPDATE orders SET order_state = 'Y', imp_uid = ? WHERE merchant_uid = ?";
      const inputData2 = [impUid, merchantUid];
      await dbcon.query(sql2, inputData2);
      ...
      const inputData3 = [merchantUid];
      const [result3] = await dbcon.query(sql3, inputData3);
      ...
      const inputData4 = [memberCode];
      await dbcon.query(sql4, inputData4);
    } catch (err) {
      return next(err);
    }

    res.sendJson({ ordered_product: orderedProduct, orders: orders });
  });
  ...
  router.get("/menu_ordered", async (req, res, next) => {
    ...
    try {
      ...
      const sql2 =
        "SELECT merchant_uid, ordered_product_name, ordered_product_img, ordered_product_count, ordered_product_price from ordered_product WHERE merchant_uid = ?";
      const inputData2 = [mer];
      const [result2] = await dbcon.query(sql2, inputData2);
      orderedProduct = result2;
